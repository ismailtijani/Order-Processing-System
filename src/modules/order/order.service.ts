...
export class OrderService extends BaseService<Order> {
  ...
  private async calculateOrderTotals(meals: any[], trx?: any) {
    ...
    for (const mealData of meals) {
      ...
      const quantity = mealData.quantity || 1;
      ...
      for (const addonData of mealData.addons || []) {
        ...
        const addon = await Addon.query(trx).findOne({
        ...
        if (!addon) {
          throw new BadRequestException(
            `Addon ${addonData.addon_meal_id} not found for meal ${mealData.meal_id}`,
          );
        }

        // Get the addon meal details
  ...
  private async generateOrderCode(): Promise<string> {
    const prefix = ORDER_CONSTANTS.ORDER_CODE_PREFIX || 'backend';
